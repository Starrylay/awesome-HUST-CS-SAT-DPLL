...
void createStartinggrid(const int a[][COL], int b[][COL], int numDigits) {//随机生成初盘
    ...
    int** c = new int* [numDigits]; //int c[numDigits][2];***********************************添加new
    for (int i = 0; i < numDigits; i++) c[i] = new int[2];//int c[numDigits][2];***********************************添加new
    ...
    for( i = 0; i < numDigits ; i++) {
        ...
        //        if (test_num != a[j][k]) {
        //            b[j][k] = test_num;
        //            string filename_temp = ToCnf(b, i);
        //            int VARNUM_temp;
        //            HeadNode* LIST_temp = CreateClause(VARNUM_temp, filename_temp);
        //            conse* SudoResult_temp = new conse[VARNUM_temp];
        //            int value = SudoDPLL(LIST_temp, SudoResult_temp, VARNUM_temp);
        //            if (value == 1)flag = 1;
        //        }
    ...
    for (int i = 0; i < numDigits; i++) delete[]c[i];//*******************************************添加delete
    delete[]c;//***********************************************添加delete
}
...
string ToCnf(int a[][COL],int holes) {
    ...
    in.close();
    return string("SAT_DPLL\\sudoku.cnf");
}
...
string createSudokuToFile() {
    ...
    cout << "***********************input the number of holes***************************" << endl;
   // int holes = 15;//挖洞个数**************************************************************默认为15
    cin >> holes;
...
void Uniquejudge(const int a[][COL], const int b[][COL])
{
    ...
    for (i = 0; i < 9; i++) {
        for (j = 0; j < 9; j++) {
            if (a[i][j] != b[i][j]) {
                cout << "this result is not unique!" << endl;
                return;
            }
        }
    }
